...
<style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .sm-table thead tr { background: var(--surface2); }
...
<div class="sm-inner">
    ...
    <div class="sm-toast" id="sm-toast">
        ...
        {{ session('success') }}
        <button class="toast-close" onclick="document.getElementById('sm-toast').remove()">✕</button>
    </div>
    @endif
    ...
    <div class="sm-grid">
        ...
        <div class="form-card">
            <div class="form-card-top">
                ...
                <span class="mode-badge {{ isset($editState) ? 'editing' : 'new' }}">
                    {{ isset($editState) ? 'Editing' : 'New' }}
                </span>
            </div>
            <div class="form-card-body">
                <form method="POST" action="{{ isset($editState) ? route('states.update', $editState->id) : route('states.store') }}">
                    ...
                    <div>
                        <label class="field-label" for="stateName">
                        ...
                        <input
                            ...
                            value="{{ isset($editState) ? $editState->name : old('name') }}"
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="state-name-cell">
                                    ...
                                    <span class="state-name-text state-name">{{ $state->name }}</span>
                                </div>
                            </td>
                            <td><span class="date-text">{{ $state->created_at->format('d M Y') }}</span></td>
                            <td style="text-align:right;">
                                <div class="action-wrap" style="justify-content:flex-end;">
                                    <a href="{{ route('states.edit', $state->id) }}" class="btn-edit-s">
                                        <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                                        Edit
                                    </a>
                                    <form action="{{ route('states.destroy', $state->id) }}" method="POST" style="margin:0" class="del-form">
                                        @csrf @method('DELETE')
                                        <button type="button" class="btn-del-s" onclick="openDeleteModal(this, '{{ $state->name }}')">
                        ...
                        <tr>
                            <td colspan="4">
                                <div class="empty-state">
                                    ...
                                    <p>No states found. Add your first state.</p>
                                </div>
                            </td>
                        </tr>
                        @endforelse
...
<div id="deleteModal" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,0.65); backdrop-filter:blur(4px); z-index:9999; align-items:center; justify-content:center;">
    <div style="background:#1a1a2a; border:1px solid #252538; border-radius:16px; padding:2rem; max-width:380px; width:90%; text-align:center;">
        ...
        <h3 style="font-family:'Syne',sans-serif;font-size:1.1rem;color:#fff;margin-bottom:0.5rem;">Delete State?</h3>
        <p style="font-size:0.82rem;color:#6b6b8a;margin-bottom:0.4rem;">You are about to delete:</p>
        <p id="deleteStateName" style="font-family:'Syne',sans-serif;font-size:0.95rem;color:#a78bfa;font-weight:600;margin-bottom:1.4rem;"></p>
...
<script>
    ...
    function openDeleteModal(btn, stateName) {
        pendingForm = btn.closest('form');
        document.getElementById('deleteStateName').textContent = stateName;
    ...
    document.getElementById('stateSearch').addEventListener('input', function () {
        ...
        const rows = document.querySelectorAll('#statesBody tr');
        let visible = 0;
        rows.forEach(row => {
            const name = row.querySelector('.state-name');
            if (!name) return;
